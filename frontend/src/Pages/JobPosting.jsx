...
function JobPosting() {
  ...
  const handleGenerate = async () => {
    ...
    // Only check essential fields for job description generation
    const essentialFields = ['title', 'experience', 'location', 'type', 'primaryRole'];
    ...
    try {
      ...
      if (error.response?.data?.message?.includes("Quota exceeded")) {
        ...
        setDialog({
          ...
          message: error.response?.data?.message || "Failed to generate job description. Please try again or check your OpenAI API key in backend.",
          buttonText: "OK",
        });
      }
